...
void mh_map_add(mh_map_t *map, mh_memory_t key, mh_memory_t value) {
    ...
    ((mh_key_value_pair_t *) this->memory->address)[this->memory->offset /
                                                    sizeof(mh_key_value_pair_t)] = (mh_key_value_pair_t) {
            .key = key,
            .value = value
    };
...
mh_memory_t mh_map_iterator_current(mh_iterator_t *iterator) {
    MH_THIS(mh_map_iterator_t*, iterator);
    return mh_memory_reference(&((mh_key_value_pair_t *) this->map->memory->address)[this->position],
                               sizeof(mh_key_value_pair_t));
}
